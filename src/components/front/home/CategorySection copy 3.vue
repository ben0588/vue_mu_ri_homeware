    ...
    <div class="container mt-5">
      <div class="category-grid">
        <div
          ...
          :class="`category-grid-${index}`"
        >
          <router-link
            :to="item.path"
            class="hover-img-opacity"
            :class="{ 'category-sale-img': item.title === '特價中' }"
          >
            <img
              :src="item.imgUrl"
              :alt="item.title"
              :style="{ width: `196x`, height: `196px` }"
              :class="{ 'category-sale-img': item.title === '特價中' }"
            />
...
.category-grid {
  ...
  /* margin: 30px; */
  display: grid;
  /* grid-template-columns: repeat(3, 1fr) 196px; */
  grid-template-columns: repeat(auto-fit, minmax(make-row(gutter: $gutter-width), 1fr));
  /* grid-template-rows: 100px 400px 200px 100px; */
  grid-gap: 15px;
